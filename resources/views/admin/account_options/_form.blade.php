@php
    ...
    $activeValue = (string) old('is_active', $option->exists ? ($option->is_active ? '1' : '0') : '1');
@endphp

<div class="ab-form">
    <p class="ab-note">
        Broker-level data (regulation, demo account, markets) lives on the
        <a href="{{ route('admin_broker_edit', $broker->id) }}">broker profile</a> — it is not duplicated here.
    </p>

    <section class="ab-section" id="profile">
        <div class="ab-section__head">
            <h2>1. Account profile &amp; costs</h2>
            <p>Name, currency, leverage, spreads, and commission shown on the review page.</p>
        </div>
        <div class="ab-section__body">
            <div class="row">
                <div class="col-md-4 form-group">
                    <label for="account_type">Account Type <span class="text-danger">*</span></label>
                    <input type="text" name="account_type" id="account_type" class="form-control @error('account_type') is-invalid @enderror" required
                    ...
                    @error('account_type')<small class="ab-error">{{ $message }}</small>@enderror
                </div>
                <div class="col-md-4 form-group">
                ...
                <div class="col-md-4 form-group">
                    ...
                    <input type="text" name="account_currency" id="account_currency" class="form-control @error('account_currency') is-invalid @enderror" required
                           value="{{ old('account_currency', $option->account_currency ?? 'USD') }}" placeholder="USD, EUR, GBP">
                    @error('account_currency')<small class="ab-error">{{ $message }}</small>@enderror
                </div>
                ...
                <div class="col-md-3 form-group">
                    ...
                    <select name="is_active" id="is_active" class="form-control">
                        ...
                        <option value="0" @selected($activeValue === '0')>Hidden</option>
                    </select>
                </div>
                <div class="col-md-3 form-group">
                    <label for="min_deposit">Minimum Deposit ($)</label>
                    <input type="number" step="0.01" name="min_deposit" id="min_deposit" class="form-control @error('min_deposit') is-invalid @enderror"
                           value="{{ old('min_deposit', $option->min_deposit) }}">
                    @error('min_deposit')<small class="ab-error">{{ $message }}</small>@enderror
                </div>
                <div class="col-md-3 form-group">
                    <label for="max_leverage_numeric">Max Leverage (numeric)</label>
                    <input type="number" name="max_leverage_numeric" id="max_leverage_numeric" class="form-control @error('max_leverage_numeric') is-invalid @enderror" min="1" max="10000"
                           value="{{ old('max_leverage_numeric', $option->max_leverage_numeric) }}" placeholder="500 for 1:500">
                    @error('max_leverage_numeric')<small class="ab-error">{{ $message }}</small>@enderror
                </div>
                <div class="col-md-4 form-group">
                ...
                <div class="col-md-4 form-group">
                    ...
                    <input type="number" step="0.01" name="spread_from_pips" id="spread_from_pips" class="form-control @error('spread_from_pips') is-invalid @enderror"
                           value="{{ old('spread_from_pips', $option->spread_from_pips ?? $option->spread_value) }}">
                    @error('spread_from_pips')<small class="ab-error">{{ $message }}</small>@enderror
                </div>
                <div class="col-md-4 form-group">
    ...
    <section class="ab-section" id="limits">
        <div class="ab-section__head">
            <h2>2. Limits, features &amp; content</h2>
            <p>Trading limits, Islamic/pro flags, and comparison copy for the review page.</p>
        </div>
        <div class="ab-section__body">
            ...
            <div class="form-group mb-0">
                ...
                <textarea name="special_conditions" id="special_conditions" class="form-control snote" data-admin-editor="compact" rows="3"
...
<div class="ab-save">
    <div class="ab-header__actions">
        <button type="submit" class="ab-btn ab-btn--primary">
            ...
            {{ $isEdit ? 'Save changes' : 'Create account option' }}
        </button>
    </div>
    <a href="{{ route('admin_account_options_index', $broker->id) }}" class="ab-btn ab-btn--ghost">Cancel</a>
</div>
