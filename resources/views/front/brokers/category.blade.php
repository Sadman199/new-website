...
@php
    // Determine if we're showing country or account type
    $isCountry = false;
    $displayName = ucfirst($slug);
    
    // You can add logic here to check if slug is a country
    // $countries = ['usa', 'uk', 'australia']; // example
    // $isCountry = in_array($slug, $countries);
    
    $pageTitle = 'Best ' . $displayName . ' Brokers | Top Accounts Compared – BrokersCourt';
    $metaDescription = 'Discover top-rated ' . $displayName . ' brokers. Compare account features, spreads, leverage, and trading conditions to choose the best option for your trading style.';
@endphp
        ...
        <div class="flex flex-col md:flex-row justify-between items-start md:items-center">
            <div>
                <h1 class="text-2xl md:text-3xl font-bold text-gray-800">
                   Best <span class="text-yellow-500">{{ $displayName }}</span> Forex Brokers {{ date('Y') }}
                </h1>
                <p class="mt-2 text-gray-600">
                    Compare regulated {{ strtolower($displayName) }} brokers with tight spreads and fast execution
                </p>
            </div>
                            ...
                            <div class="flex items-center space-x-3">
                                <div>
                                    ...
                                    <p class="text-gray-800 font-medium">
                                        ...
                                        <strong class="text-yellow-500">{{ $displayName }}</strong> forex broker{{ $brokers->count() !== 1 ? 's' : '' }} available
                                    </p>
                                </div>
                            </div>
        ...
        <div class="flex flex-col sm:flex-row sm:items-end justify-between gap-4">
            <div>
                <h2 class="text-2xl sm:text-3xl font-bold text-gray-900 tracking-tight">
                    ...
                    <span class="uppercase">{{ $displayName }}</span> FOREX BROKERS
                </h2>
        ...
        @foreach($top_brokers as $broker)
            <div class="p-4 mb-4 border rounded-lg">
                ...
                <div class="flex items-center gap-4">
                    @if($broker->logo)
                        <img src="{{ asset('uploads/broker/' . $broker->logo) }}"
                            alt="{{ $broker->name }}"
                            class="object-contain w-16 h-16">
                    @endif

                    <div>
                        <h3 class="text-lg font-semibold">{{ $broker->name }}</h3>
                        <p class="text-sm text-gray-600">
                            Rating: {{ $broker->rating }}/5
                        </p>
                    </div>
                </div>
        ...
        <div class="grid md:grid-cols-3 divide-x divide-gray-200">
            ...
            <div class="p-6">
                ...
                <div class="flex flex-wrap gap-2 mt-2">
                    <span class="inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium bg-blue-100 text-blue-800">MT4</span>
                    <span class="inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium bg-indigo-100 text-indigo-800">MT5</span>
                    <span class="inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium bg-purple-100 text-purple-800">cTrader</span>
                    <span class="inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium bg-green-100 text-green-800">Web Terminal</span>
                </div>
        ...
        <div class="bg-gray-50 px-6 py-3 border-t border-gray-200">
            <div class="flex items-center justify-between">
                <div class="flex items-center text-sm text-gray-500">
                    ...
                    All brokers verified on {{ now()->format('F j, Y') }}
                </div>
                <a href="{{ route('broker.comparison') }}">
                    <button class="inline-flex items-center px-4 py-2 bg-gradient-to-r from-yellow-500 to-yellow-600 hover:from-yellow-600 hover:to-yellow-700 rounded-md text-sm font-medium text-white transition-colors duration-200 shadow-sm hover:shadow-md">
                        Compare All Brokers
                        <svg xmlns="http://www.w3.org/2000/svg" class="ml-1 -mr-0.5 h-3 w-3" viewBox="0 0 20 20" fill="currentColor">
