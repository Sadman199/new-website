        ...
        <div class="flex flex-col md:flex-row justify-between items-start md:items-center">
            <div>
                <h1 class="text-2xl md:text-3xl font-bold text-gray-800">
                   Best <span class="text-yellow-500">{{ ucfirst($country) }}</span> Forex Brokers {{ date('Y') }}
                </h1>
                <p class="mt-2 text-gray-600">
                    Compare regulated {{ strtolower($country) }} brokers with tight spreads and fast execution
                </p>
            </div>
    ...
    <div class="container max-w-7xl mx-auto px-4">
        <div class="grid grid-cols-1 lg:grid-cols-12 gap-8">
            ...
            <div class="lg:col-span-8">
                @if($paginatedBrokers->isNotEmpty())
                    <div class="mb-6 p-4 bg-gradient-to-r from-gray-50 to-gray-100 border border-gray-200 rounded-xl shadow-xs">
                                ...
                                <div>
                                    ...
                                    <p class="text-gray-800 font-medium">
                                        <span class="text-2xl font-bold text-gray-700">
                                            {{ $paginatedBrokers instanceof \Illuminate\Pagination\LengthAwarePaginator
                                                ? number_format($paginatedBrokers->total())
                                                : number_format($paginatedBrokers->count()) }}
                                        </span>
                                        <strong class="text-yellow-500">{{ ucfirst($country) }}</strong>
                                        forex broker{{ $paginatedBrokers->count() !== 1 ? 's' : '' }} available
                                    </p>
                                </div>
                            </div>

                            @if($paginatedBrokers instanceof \Illuminate\Pagination\LengthAwarePaginator)
                                <div class="hidden sm:block px-3 py-1 bg-white rounded-full border border-gray-200">
                                    <span class="text-xs font-medium text-gray-600">
                                        Viewing <span class="text-amber-600">
                                            {{ $paginatedBrokers->firstItem() }}-{{ $paginatedBrokers->lastItem() }}
                                        </span> of {{ $paginatedBrokers->total() }}
                                    </span>
                                </div>
                            @endif
                        </div>
                     ...
                     @if ($paginatedBrokers->count() > 0)
                        <div class="space-y-4">
                            ...
                            <!-- Pagination -->
                            @if ($paginatedBrokers instanceof \Illuminate\Pagination\LengthAwarePaginator && $paginatedBrokers->hasPages())
                                <div class="mt-8 flex items-center justify-center space-x-1 mb-12">
                                    {{-- Previous Page Link --}}
                                    @if ($paginatedBrokers->onFirstPage())
                                        <span class="px-3 py-1 rounded-md text-gray-400 cursor-not-allowed text-sm">&laquo;</span>
                                    @else
                                        <a href="{{ $paginatedBrokers->previousPageUrl() }}"
                                    ...
                                    {{-- Page Numbers --}}
                                    @foreach ($paginatedBrokers->getUrlRange(1, $paginatedBrokers->lastPage()) as $page => $url)
                                        @if ($page == $paginatedBrokers->currentPage())
                                    ...
                                    {{-- Next Page Link --}}
                                    @if ($paginatedBrokers->hasMorePages())
                                        <a href="{{ $paginatedBrokers->nextPageUrl() }}"
                                           class="px-3 py-1 rounded-md text-white bg-gradient-to-r from-yellow-500 to-yellow-600 hover:from-yellow-600 hover:to-yellow-700 text-sm font-medium transition-all duration-200">
                                            &raquo;
                                        </a>
                                    @else
        ...
        <div class="flex flex-col sm:flex-row sm:items-end justify-between gap-4">
            <div>
                <h2 class="text-2xl sm:text-3xl font-bold text-gray-900 tracking-tight">
                    ...
                    <span class="uppercase">{{ $country }}</span> FOREX BROKERS
                </h2>
    ...
    <div class="bg-white border border-gray-200 rounded-xl shadow-sm overflow-hidden mt-12">
        ...
        <div class="flex items-center justify-between">
            ...
            <h2 class="text-2xl font-bold text-gray-800">
                Premium {{ ucfirst($country) }} Brokers
