    ...
    <div class="border-t border-gray-200 px-4 py-3 bg-gray-50">
        ...
        <div class="mt-2">
            <div id="summary-{{ $broker->id }}" class="tab-content active">
                <div class="grid grid-cols-2 md:grid-cols-4 gap-3 md:gap-4 text-xs md:text-sm">
                    ...
                    <div>
                        ...
                        <div class="font-medium">{{ ($platforms = $broker->platformList()) ? implode(', ', array_slice($platforms, 0, 2)) : '—' }}</div>
                    </div>
                    <div>
                        <div class="text-gray-500">Assets</div>
                        <div class="font-medium">{{ $broker->instrument_count ? $broker->instrument_count . '+' : '100+' }}</div>
                    </div>
                    <div>
            ...
            <div class="text-xs md:text-sm text-gray-700">
                @php $regs = $broker->regulationList(); @endphp
                {{ $regs ? implode(', ', $regs) : '—' }}
                </div>
            </div>
        </div>
