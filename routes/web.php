...
Route::middleware('guest:author')->group(function () {
    ...
    Route::post('/author/login', [AuthorLoginController::class, 'login'])->middleware('throttle:5,1')->name('author_login_submit');
});

Route::post('/author/logout', [AuthorLoginController::class, 'logout'])->middleware('author:author')->name('author_logout');

Route::middleware('author:author')->prefix('author')->group(function () {
...
Route::get('/admin/ads/create', [AdminAdController::class, 'create'])->name('admin_ads_create')->middleware('admin:admin');
...
Route::group(['prefix' => 'admin/users', 'middleware' => 'admin:admin'], function () {
    ...
    Route::get('/{id}', [AdminUserController::class, 'show'])->whereNumber('id')->name('admin_users_show');
    Route::patch('/{id}/verify', [AdminUserController::class, 'verify'])->name('admin_users_verify');
    Route::patch('/{id}/unverify', [AdminUserController::class, 'unverify'])->name('admin_users_unverify');
    Route::patch('/{id}/toggle-status', [AdminUserController::class, 'toggleStatus'])->name('admin_users_toggle_status');
    Route::delete('/{id}', [AdminUserController::class, 'destroy'])->name('admin_users_delete');
});
// ==== User Management End ====

Route::group(['prefix' => 'admin/account-options', 'middleware' => 'admin:admin'], function () {
    Route::get('/', [AccountOptionController::class, 'all'])->name('admin_account_options_all');
});
...
Route::redirect('/brokers', '/find-my-broker', 301)->name('all_brokers');
Route::redirect('/brokers/filter', '/find-my-broker', 301)->name('all_brokers_filter');
...
Route::get('/scam-brokers/{slug}', [ScamBrokerController::class, 'show'])->name('scam_broker_detail');

// ==== User Accounts (front-end) ====
