...
$(document).ready(function() {
    ...
    $("#brokerSlider1, #brokerSlider2").owlCarousel({
        items: 1,
        loop: true,
        autoplay: true,
    ...
    $('.select-broker').on('click', function() {
        var brokerSlug = $(this).data('value');
        var brokerId = $(this).data('id');

        // Update hidden input field
        $('#compare_broker' + brokerId).val(brokerSlug);

        // Change button text
        if (brokerId === 1) {
            $('#compare_broker1_toggle').text('Broker 1: ' + $(this).closest('.card').find('.card-title').text());
        } else {
            $('#compare_broker2_toggle').text('Broker 2: ' + $(this).closest('.card').find('.card-title').text());
        }
    });

    // Form validation before submission
    $('#compareForm').on('submit', function(e) {
        var broker1 = $('#compare_broker1').val();
        var broker2 = $('#compare_broker2').val();

        if (!broker1 || !broker2) {
            e.preventDefault();  // Prevent form submission
            if (!broker1) {
                $('#compare_broker1_error').show();
            } else {
                $('#compare_broker1_error').hide();
            }
            if (!broker2) {
                $('#compare_broker2_error').show();
            } else {
                $('#compare_broker2_error').hide();
            }
        }
    });
});
